...
//
// Why booking needs this at all: a made-up address does not just cost us a junk
// message, it puts an event in the real calendar and sends an invitation into
// nowhere. Proof-of-work cannot tell a typo from a lie; a code in the inbox can.
//
// Byte-identical to kchat-api/src/otp.ts. Two edge scripts deploy separately and
// neither may import across the other's bundle, so this is copied rather than
// shared - same reasoning as challenge.ts, which already lives in both.
...
export const verifyCode = async (
    ...
    spent.set(tokenStr, now); // single-use: one verified code books one slot

...
if (localPort) {
    console.log(`[booking-api] local server on http://127.0.0.1:${localPort}`);
    console.log(`[booking-api] timezone ${config.timezone}, origins ${config.allowedOrigins.join(", ")}`);
    BunnySDK.net.http.serve({ port: localPort, hostname: "127.0.0.1" }, handler);
} else {
    console.log(`[booking-api] edge mode, timezone ${config.timezone}`);
